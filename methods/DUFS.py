...
def full_affinity_knn(X, knn=2,fac=0.6):
    ...
    mu = tfp.stats.percentile(knn_distances, 50., interpolation='higher')
    ml = tfp.stats.percentile(knn_distances, 50., interpolation='lower')
    sigma=(mu+ml)/2.
    sigma=tf.cond(tf.less(sigma,1e-8),lambda:1.,lambda:sigma)
    W = tf.keras.backend.exp(-Dx/ (fac*sigma) )
    Dsum=tf.keras.backend.sum(W,axis=1)
    Dminus=tf.keras.backend.pow(Dsum,-1)
...
class Model(object): 
     def __init__(self,
        ...
        with G.as_default():
            ...
            # tf Graph Input
            X = tf.compat.v1.placeholder(tf.float32, [None, input_dim]) # i.e. mnist data image of shape 28*28=784            
            
            self.learning_rate= tf.compat.v1.placeholder(tf.float32, (), name='learning_rate')
            
            self.nnweights = []
            masked_input = X
            with tf.compat.v1.variable_scope('concrete', reuse=tf.compat.v1.AUTO_REUSE):
                self.alpha = tf.compat.v1.get_variable('alpha', [input_dim,],
            ...
            reg = 0.5 - 0.5*tf.math.erf((-1/2 - input2cdf)/(self.sigma*np.sqrt(2)))
     ...
     def feature_selector(self, x):
        ...
        :return: gated input
        '''
        base_noise = tf.compat.v1.random_normal(shape=tf.shape(x), mean=0., stddev=1.)
